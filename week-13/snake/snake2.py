...
class Wall:
  ...
  def collision(self, snake): 
      x, y = snake[-1]
      for xi, yi in self.body: 
          if xi * block == x and yi * block == y: 
              #print("TRUE WORKING: ", str(x), str(y))
              return True
    ...
    while menu:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and 430 <= mx <= 550 and 570 <= my <= 600:
                global user, record
                user = input()
                select = "SELECT level FROM public.snake WHERE name = '{0}';"
...
while True:
    ...
    if wall.collision(snake) or (len(snake) > len(set(snake))):
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    if record != None: 
                        ...
                        conn.commit()
                        insert = "INSERT INTO public.snake(name, level) VALUES ('{0}', {1});" 
                        cur.execute(insert.format(user, Level))
                        conn.commit()
                    else: 
                        insert = "INSERT INTO public.snake(name, level) VALUES ('{0}', {1});"
                        cur.execute(insert.format(user, Level))
    ...
    if snake[-1] == apple:
        while apple in snake:
            ...
            score += randint(1, 3)
            food_number += 1
            if food_number % 4 == 0 and food_number != 0 and food_number < 12:
                Level += 1
                wall.load_wall()
                fps += 3
